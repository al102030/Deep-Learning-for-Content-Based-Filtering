...
print_existing_user(sorted_ypu, sorted_y.reshape(-1, 1),
                    sorted_user, sorted_items, ivs, uvs, movie_dict, maxcount=50)
...
input_item_m = tf.keras.layers.Input(
    shape=(num_item_features))    # input layer
...
m_dist = ma.masked_array(dist, mask=np.identity(
    dist.shape[0]))  # mask the diagonal
